...
@section('content')
    ...
    <form action="{{ route('skrydis.index') }}" method="GET">
        @csrf
        <input class="mygtukas" type="date" name="data">
        <select type="text" name="vieta" >
    ...
    @foreach ($list as $skrydis)
        ...
        <div class="admin-panel">
            ...
            <a href="{{ route('skrydis.show', $skrydis)  }}">
                <span class="spankodas">{{ $skrydis->kodas }}</span><span class="vardas">{{ $skrydis->vardas }}</span><span> {{$skrydis->pastas}}</span> <span>{{$skrydis->telefonas}}</span>
